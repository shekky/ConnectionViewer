...
{
    public partial class ConnectionViewerForm : Form
    {
        ...
        public static Dictionary<String, HostDetailsRecord> HostInfoMap;
        public static Queue<String> HostDetailsQueue;

        public static Object Lock = new object();

        public ConnectionViewerForm()
        {
            // Intialize the components on the Windows Form.
            InitializeComponent();

            HostInfoMap = new Dictionary<String, HostDetailsRecord>();
            HostDetailsQueue = new Queue<String>();

            StartHostInfoThread();
        }

        public void StartHostInfoThread()
        {
            String ipAddress;
            Thread th = new Thread(() =>
            {
                
                HostDetailsRecord rec;
                while (true) {
                    ...
                    lock (Lock)
                    {
                        ...
                        if (HostDetailsQueue.Count() == 0)
                        continue;

                        ipAddress = HostDetailsQueue.Dequeue();

                        rec = new HostDetailsRecord();

                        rec.WhoisRecord = new HostDetails.Whois().LookupIp(ipAddress.ToString());

                        try { 
                            IPHostEntry entr = Dns.GetHostEntry(IPAddress.Parse(ipAddress));
                            rec.DnsRecord = entr.HostName;
                        }
                        catch (Exception exc)
                        {
        ...
        public struct HostDetailsRecord
        {
            public HostDetails.WhoisRecord WhoisRecord { set; get; }
            public string DnsRecord{ set; get; }
        ...
        /// </summary>
        private void ConnectionViewer_Load(object sender, EventArgs e)
        {
    ...
    public class TcpProcessRecord
    {
        ...
        [DisplayName("Remote Domain Name")]
        public string RemoteDomainName { get; set; }
        [DisplayName("Remote Country")]
        public string RemoteCountry { get; set; }
        [DisplayName("Remote Orginization Name")]
